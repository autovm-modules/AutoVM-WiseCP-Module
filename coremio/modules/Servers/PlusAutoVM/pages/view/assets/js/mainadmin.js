const app = Vue.createApp({
    ...
    methods: {
        ...
        async getMachineIDFromDataBase() {
            try {
                const response = await axios.get(this.baseApiUrl, {
                    params: {
                        operation: "operation_server_automation",
                        use_method: "getMachineidFromDatabase"
                    },
                });

                if (response.data) {
                    let result = response.data
                    this.machineIdFromDataBase = result
                }
            } catch (error) {
                console.error('getMachineidFromDatabase in vue didnt work:', error);
            }
        ...
        // SET Machine ID
        async changeMachineID() {

            let newMachineID = this.newMachineID;

            if (newMachineID && Number.isInteger(newMachineID) && newMachineID > 0) {
                let response = await axios.get(this.baseApiUrl, {
                    params: {
                        operation: "operation_server_automation",
                        use_method: "changeMachineID",
                        newId: newMachineID
                    },
                });
                ...
                console.log('new ID is not a number');
            }
        },
        ...
        async loadBandwidth() {
            try {
                const response = await axios.get(this.baseApiUrl, {
                    params: {
                        operation: "operation_server_automation",
                        use_method: "currentBandwidthUsage"
                    },
                });
